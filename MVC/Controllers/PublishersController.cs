...
namespace MVC.Controllers
{
    ...
    public class PublishersController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            var list = _publisherService.List();
        ...
        public IActionResult Details(int id)
        {
            ...
            var item = _publisherService.Item(id);
        ...
        public IActionResult Create(PublisherRequest publisher)
        {
            if (ModelState.IsValid) // check data annotation validation errors in the request
            {
                // Insert item service logic:
                var response = _publisherService.Create(publisher);
            ...
            SetViewData(); // set ViewData dictionary to carry extra data other than the model to the view
            return View(publisher); // return request as model to the Create view
        }
        ...
        public IActionResult Edit(int id)
        {
            ...
            var item = _publisherService.Edit(id);
            SetViewData(); // set ViewData dictionary to carry extra data other than the model to the view
        ...
        [HttpPost, ValidateAntiForgeryToken]
        public IActionResult Edit(PublisherRequest publisher)
        {
            if (ModelState.IsValid) // check data annotation validation errors in the request
            {
                // Update item service logic:
                var response = _publisherService.Update(publisher);
                if (response.IsSuccessful)
                {
                    SetTempData(response.Message); // set TempData dictionary to carry the message to the redirected action's view
                    return RedirectToAction(nameof(Details), new { id = response.Id }); // redirect to Details action with id parameter as response.Id route value
            ...
            SetViewData(); // set ViewData dictionary to carry extra data other than the model to the view
            return View(publisher); // return request as model to the Edit view
        }
        ...
        public IActionResult DeleteConfirmed(int id)
        {
            ...
            var response = _publisherService.Delete(id);
            SetTempData(response.Message); // set TempData dictionary to carry the message to the redirected action's view
